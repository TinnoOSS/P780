...
//for debug pony 20210508
#include <linux/thermal.h>

//pony.ma, DATE20210511, stop charging when reach set soc for factory
...
//for scc by pony start date20210420
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	#ifdef CONFIG_SCC_SUPPORT
	int byte_len_speed;				//by pony.ma for scc date20210420
	#endif
...
static int smb5_parse_dt_currents(struct smb5 *chip, struct device_node *node)
{
	...
	//for dcp icl config by pony date20210414-01 start
	rc = of_property_read_u32(node,
				"dcp-icl-en", &chg->dcp_icl_en);
	if (rc < 0)
		chg->dcp_icl_en = -EINVAL;
	...
	//for dcp icl config by pony date20210414-01 end
...
//pony.ma, DATE20210511, stop charging when reach set soc for factory start
#ifdef CONFIG_TINNO_DEMOMODECHG_CONTROL
...
**********************************************************************************************************/
struct smb_charger *demomode_chg = NULL;
int demomode_enable = 0, demomode_max_soc = 80, demomode_min_soc = 40;
static ssize_t demomode_enable_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    ...
    if (sscanf(desc, "%d", &enable) == 1)
    {
		...
		demomode_chg->demomode_enable = enable;
		return count;
    }
...
static enum power_supply_property smb5_usb_props[] = {
	...
	//for debug pony 20210508 start
	POWER_SUPPLY_PROP_QUICK_CHARGE_TYPE,
	POWER_SUPPLY_PROP_TEMP,
	//for debug pony 20210508 end
};
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		//for scc by pony start date20210420
		#ifdef CONFIG_SCC_SUPPORT
		if((val->intval) && (last_speed != target_speed)){
	...
	case POWER_SUPPLY_PROP_TEMP:
			{
				...
				thermal_dev = thermal_zone_get_zone_by_name("chg-skin-therm-usr");
...
static enum power_supply_property smb5_batt_props[] = {
	POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT,
	POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED,          //add charging enable node by pony.ma date20210414
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		...
		break;
	//add charging enable node by pony.ma date20210414
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		//pony.ma, DATE20210511, stop charging when reach set soc for factory
		#ifdef CONFIG_TINNO_DEMOMODECHG_CONTROL
		if(demomode_enable && (pval->intval >= demomode_max_soc) && (chg->real_charger_type != POWER_SUPPLY_TYPE_UNKNOWN))
			vote(chg->chg_disable_votable, DEMOMODE_VOTER, true, 0);
		if(demomode_enable && (pval->intval <= demomode_min_soc) && (chg->real_charger_type != POWER_SUPPLY_TYPE_UNKNOWN))
			vote(chg->chg_disable_votable, DEMOMODE_VOTER, false, 0);
		#endif
		...
		const union power_supply_propval *val)
{
	...
	//add charging enable node by pony.ma date20210414
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		vote(chg->chg_disable_votable, CHG_NODE_VOTER, !val->intval, 0);
		break;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		...
		enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
	case POWER_SUPPLY_PROP_CAPACITY:
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:    //add charging enable node by pony.ma date20210414
		return 1;
	default:
		break;
	}
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	//for match purple wire by pony date20210518 start
	/* Rp-Rp DAM cable */
	...
	//for match purple wire by pony date20210518 end
...
static int smb5_determine_initial_status(struct smb5 *chip)
{
	...
	msleep(250);						/*resolve typec detect fail when poweron with usb date20210603*/
