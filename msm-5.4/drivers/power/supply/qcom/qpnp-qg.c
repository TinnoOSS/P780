...
//pony.ma avoid battery deterioration due to over-discharge date20210417--start
#ifdef CONFIG_LOWBATT_PWROFF
extern char *saved_command_line;
#define CHARGER_MODE_BOOT   "androidboot.mode=charger"
#endif
//pony.ma avoid battery deterioration due to over-discharge date20210417--end
...
static int qg_debug_mask = QG_DEBUG_PROFILE | QG_DEBUG_IRQ | QG_DEBUG_PON | QG_DEBUG_DEVICE | QG_DEBUG_SOC;
...
//add for TinnoProductInfo by pony DATE20210419
#ifdef CONFIG_TINNO_PRODUCT_INFO
#include <dev_info.h>
int bat_info,bat_temp,chg_counter;	/*chg_counter is charge counter for custom require pony20210609*/
int get_battery_vol_info(char *buf, void *arg0) {
	pr_err("bat_info=%d,bat_temp=%d,chg_count=%d\n", bat_info,bat_temp,chg_counter);
	return sprintf(buf, "%dmv,%dT,%dN", bat_info,bat_temp,chg_counter);
}
#endif

// QG trace information, DATE20210419-01 START
static int qg_get_battery_capacity(struct qpnp_qg *chip, int *soc);
void qg_status_trace(struct qpnp_qg *chip)
{
	...
	if (usb_psy1) {
		...
		if (rc < 0)
			pr_err("Couldn't read USB usb_i_now, rc=%d\n", rc);
		else
	...
	#ifdef CONFIG_TINNO_PRODUCT_INFO
	 rc = get_cycle_count(chip->counter, &counter);
	 bat_info = chip->sdam_data[SDAM_OCV_UV]/1000;
	 bat_temp = chip->sdam_data[SDAM_TEMP];
	 chg_counter = counter;
	...
	if (curr_soc != last_soc)
	{
		...
		qg_dbg(chip, QG_DEBUG_SOC, "T CHGING = %d, UI_SOC = %d, OCV = %d, VC = %d, I = %d, USB_V = %d, USB_I = %d, T = %d, percent_time = %lu, total_time = %lu\n",
	...
	{
		qg_dbg(chip, QG_DEBUG_SOC, "M CHGING = %d, UI_SOC = %d, OCV = %d, VC = %d, I = %d, USB_V = %d, USB_I = %d, T = %d, batt_soc = %d, cc_soc = %d, charge_full = %d, charge_done = %d\n",
			(is_usb_present(chip) ? 1 : 0), 		// plug/unplug
			curr_soc,								// %
			chip->sdam_data[SDAM_OCV_UV]/1000,		// mV
			vbat/1000,								// mV
...
static void process_udata_work(struct work_struct *work)
{
	...
	// QG trace information, DATE20210419
...
static int qg_charge_full_update(struct qpnp_qg *chip)
{
	...
	qg_dbg(chip, QG_DEBUG_SOC, "msoc=%d health=%d charge_full=%d charge_done=%d\n",
				chip->msoc, health, chip->charge_full,
				chip->charge_done);
	if (chip->charge_done && !chip->charge_full) {
		if (chip->msoc >= 99 && health == POWER_SUPPLY_HEALTH_GOOD) {
			chip->charge_full = true;
			qg_dbg(chip, QG_DEBUG_SOC, "Setting charge_full (0->1) @ msoc=%d\n",
					chip->msoc);
		...
		if ((chip->wa_flags & QG_RECHARGE_SOC_WA) &&
			...
			if (rc < 0)
				...
				qg_dbg(chip, QG_DEBUG_SOC, "Forced recharge\n");
		}
		...
		if (chip->msoc < recharge_soc || !input_present) {
			...
			qg_dbg(chip, QG_DEBUG_SOC, "msoc=%d recharge_soc=%d charge_full (1->0)\n",
					chip->msoc, recharge_soc);
		} else {
			/* continue with charge_full state */
			qg_dbg(chip, QG_DEBUG_SOC, "msoc=%d recharge_soc=%d charge_full=%d input_present=%d\n",
					chip->msoc, recharge_soc,
