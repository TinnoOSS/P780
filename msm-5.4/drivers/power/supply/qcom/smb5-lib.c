...
static const struct apsd_result *smblib_update_usb_type(struct smb_charger *chg)
{
	...
	#ifdef CONFIG_TINNO_DEMOMODECHG_CONTROL
	rc = smblib_get_prop_from_bms(chg, SMB5_QG_CAPACITY, &demomode_cap);
	if (rc < 0)
		smblib_err(chg, "Couldn't get capacity prop rc=%d\n", rc);

	if((chg->demomode_enable) && (apsd_result->name != 0) && (demomode_cap < 80))
				...
				const union power_supply_propval *val)
{
	...
	//zhang.chao add for usb current limit
...
int smblib_run_aicl(struct smb_charger *chg, int type)
{
	...
	smblib_dbg(chg, PR_REGISTER, "re-running AICL\n");
				    ...
				    union power_supply_propval *val)
{
	...
	val->intval = MICRO_9V;	/*for hvdcp debug by pony date20210519*/
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	case SNK_RP_STD_BIT:
	//for match purple wire by pony date20210518
	...
	if (chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (usb_current == -ETIMEDOUT) {
			...
			if (chg->connector_type ==
				...
				if(chg->dcp_icl_en)
					rp_ua = chg->float_icl_ua;
				smblib_dbg(chg, PR_MISC, "float_icl_ua = %d\n",rp_ua);
...
static void update_sw_icl_max(struct smb_charger *chg, int val)
{
	...
	case POWER_SUPPLY_TYPE_USB_DCP:
		...
		//for dcp icl config by pony date20210414-01 start
		if(chg->dcp_icl_en)
			rp_ua = chg->dcp_icl_ua_1;
		smblib_dbg(chg, PR_MISC, "dcp_icl_ua_1 %d\n",rp_ua);
		//for dcp icl config by pony date20210414-01 end
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, rp_ua);
		break;
