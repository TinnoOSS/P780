...
static void st54spi_power_off(struct st54spi_data *st54spi)
{
	...
	/*** lianghai.shi modify by quallcomm 
	...
	***/
	...
	if (st54spi->power_or_nreset_gpio)
		gpio_direction_output(st54spi->power_or_nreset_gpio, 0);
...
static void st54spi_power_on(struct st54spi_data *st54spi)
{
	...
	if (st54spi->power_or_nreset_gpio) {
		gpio_direction_output(st54spi->power_or_nreset_gpio, 1);
...
static const struct spi_device_id st54j_spi_id[] = {
    {"st54spi", 0},
    {},
};
MODULE_DEVICE_TABLE(spi, st54j_spi_id);
...
static int st54spi_parse_dt(struct device *dev, struct st54spi_data *pdata)
{
	...
	if (np) {
		...
		r = of_get_named_gpio(np, "gpio-power-nreset-std", 0);
		if (r < 0)
	...
	pdata->pctrl_mode_spi = pinctrl_lookup_state(pdata->pctrl, "qupv3_se2_spi_active");
	if (IS_ERR(pdata->pctrl_mode_spi)) {
		dev_err(dev, "%s: Unable to find qupv3_se2_spi_active: %d\n",
			__FILE__, PTR_ERR(pdata->pctrl_mode_spi));
		return -1;
	}

	pdata->pctrl_mode_idle = pinctrl_lookup_state(pdata->pctrl, "qupv3_se2_spi_sleep");
...
static int st54spi_probe(struct spi_device *spi)
{
	...
	if (st54spi->power_or_nreset_gpio != 0) {
		...
		if (ret)
			dev_err(&spi->dev, "%s : reset direction_output failed\n", __FILE__);
...
static int __init st54spi_init(void)
{
	...
	if (status < 0) {
		pr_err("ST54J spi_register_driver failed: %d\n", status);
