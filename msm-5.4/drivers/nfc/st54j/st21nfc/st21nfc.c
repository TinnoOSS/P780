...
// QCOM and MTK54 use standard GPIO definition
	...
	np = of_find_compatible_node(NULL, NULL, "st,st21nfc");
  	if (!np) {
    	pr_err("%s : cannot find st,st21nfc in DTS.\n", __func__);
    	return -ENODEV;
  	}
#endif
...
// QCOM and MTK54 use standard GPIO definition
	...
	if (ret) {
  	...
  	if (ret) {
    	pr_err("%s : irq gpio_direction_input failed\n", __FILE__);
    	return -ENODEV;
  	}
#endif
...
#else
	r = of_get_named_gpio(np, "st,reset-gpios", 0);
	if (!gpio_is_valid(r)) {
    	pr_err("%s: get NFC RESET GPIO failed (%d)", __FILE__, r);
    	return -ENODEV;
  	}
  	st21nfc_dev->gpiod_reset = gpio_to_desc(r);	
	ret = gpio_request(r, "gpio-rst");
	if (ret) {
...
#else
	r = of_get_named_gpio(np, "st,pidle-gpios", 0);
	if (!gpio_is_valid(r)) {
    	pr_warn("[OPTIONAL] Unable get NFC PIDLE GPIO (%d)", r);
		ret = 0;
  	} else {
  		st21nfc_dev->gpiod_pidle = gpio_to_desc(r);	
	}
#endif
...
#else
	r = of_get_named_gpio(np, "st,clkreq-gpios", 0);
	if (!gpio_is_valid(r)) {
    	pr_err("%s: get NFC CLKREQ GPIO failed (%d)", __FILE__, r);
    	return -ENODEV;
  	}
  	st21nfc_dev->gpiod_clkreq = gpio_to_desc(r);	
	ret = gpio_request(r, "gpio-clkreq");
	if (ret) {
    	pr_err("%s : clkreq gpio_request failed\n", __FILE__);
    	return -ENODEV;
  	}
  	pr_info("%s : CLK REQ GPIO = %d\n", __func__, r);
  	ret = gpio_direction_input(r);
  	if (ret) {
    	pr_err("%s : clk req gpio_direction_input failed\n", __FILE__);
    	return -ENODEV;
  	}
#endif
