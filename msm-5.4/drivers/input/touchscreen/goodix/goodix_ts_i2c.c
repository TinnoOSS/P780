...
//feiyu.zhu add for TP suspend/resume
#ifdef CONFIG_DRM
...
//feiyu.zhu add for TP suspend/resume
#ifdef CONFIG_DRM
int goodix_ts_check_dt(struct device_node *np)
{
	int i;
	int count;
	...
	for (i = 0; i < count; i++) {
		...
		if (!IS_ERR(panel)) {
			...
			ts_info("%s:find", __func__);
			return 0;
		}
	}

	ts_info("%s: not find", __func__);
	return -ENODEV;
}

int goodix_ts_check_default_tp(struct device_node *dt, const char *prop)
{
	const char **goodix_active_tp = NULL;
	int count = 0;
	int tmp = 0;
	int score = 0;
	const char *active;
	...
	goodix_active_tp = kcalloc(count, sizeof(char *),  GFP_KERNEL);
	if (!goodix_active_tp) {
		ts_err("Goodix alloc failed\n");
		return -ENOMEM;
	}

	ret = of_property_read_string_array(dt->parent, prop,
			goodix_active_tp, count);
	if (ret < 0) {
...
//feiyu.zhu add for TP suspend/resume
	...
	if (ret) {
		if (!goodix_ts_check_default_tp(dp, "qcom,i2c-touch-active"))
			ret = -EPROBE_DEFER;
		else
			ret = -ENODEV;
