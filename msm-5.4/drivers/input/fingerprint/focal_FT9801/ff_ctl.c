...
extern int ff_ctl_abnormal_power_reset_device(void);//lijian add reset for Power on again abnormal.
...
static int ff_ctl_init_driver(void)
{
    ...
    #else//lijian add reset for Power on again abnormal.
    err = ff_ctl_abnormal_power_reset_device();
    #endif
...
static int __init ff_ctl_driver_init(void)
{
    ...
    ff_ctl_context.wake_lock = wakeup_source_create("fp_wakelock");
