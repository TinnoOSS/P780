					...
					struct spi_master *spi)
{
	...
	//lianghai.shi modify for NFC SPI CS to CLK delay > 10us(15clock) begin:
	if (!strcmp(dev_name(mas->dev), "4a88000.spi")){
		geni_write_reg(0xF<<10, mas->base, SE_SPI_DELAY_COUNTERS);
	} else {
	geni_write_reg(spi_delay_params, mas->base, SE_SPI_DELAY_COUNTERS);
	}
	//lianghai.shi modify for NFC SPI CS to CLK delay > 10us end
	GENI_SE_DBG(mas->ipc, false, mas->dev,
